...
void Map::PrintStatistic()
{
    ...
    if (worldStat && !worldStat->empty())
    {
        ...
        sLog.outError("World::Statistic %s",result.c_str());
        worldStat->clear();
    }

    sLog.outError("Map::Statistic map (id %u, inst %u, diff %u): AVG exec time %f ms, objects/type: %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u %u/%u, auras %u, holders %u",
