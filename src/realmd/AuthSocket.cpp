...
bool AuthSocket::_HandleLogonChallenge()
{
    ...
    else
    {
        ...
        if (result)
        {
            ...
            if (!locked)
            {
                uint32 accId = (*result)[1].GetUInt32();
            ...
            if (QueryResult *loginfail = LoginDatabase.PQuery("SELECT id, failed_logins FROM account WHERE username = '%s'", _safelogin.c_str()))
            {
                ...
                if ( failed_logins >= MaxWrongPassCount )
                {
                    ...
                    if (WrongPassBanType)
                    {
                        ...
                        LoginDatabase.PExecute("INSERT INTO account_banned VALUES ('%u',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','Login Server','Failed login autoban',1)",
                    ...
                    else
                    {
                        ...
                        LoginDatabase.PExecute("INSERT INTO ip_banned VALUES ('%s',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','Login Server','Failed login autoban')",
